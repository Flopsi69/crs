(function () {
  ...
  const styles = /* css */ `
    ...
    .lav-product--currency .lav-product__title {
      display: block;
    }
    ...
    .lav-product__divider {
      margin-top: auto;
      background: linear-gradient(90deg, hsl(var(--border) / 0.1) 0%, hsl(var(--border)) 50%, hsl(var(--border) / 0.1) 100%);
    }
    .lav-product__price {
  ...
  stylesEl.classList.add('exp-blog-v2-styles');
  ...
  async function addSidebar(parentEl, gameName, type) {
    ...
    config.items = config.items.slice(0, 10)
    ...
    const markup = /* html */ `
      ...
      <div class='lav-sidebar__btn-wrap'>
        <div class='lav-sidebar__btn'>
          View All ${config.name} ${config.game?.currency_name || type} ${getSvg('arrow-right')}
        </div>
      </div>
    `
    ...
    config.items.forEach(item => {
      ...
      if (type === 'accounts') {
        productEl = getProductAccount('sidebar', item, config.game.url, type)
      } else if (type === 'items') {
        productEl = getProductItem('sidebar', item, config.game.url, type)
      } else {
        productEl = getProductCurrency('sidebar', item, config.game.url, type)
      }
  ...
  function updateBannerStyle() {
    _$$('.lav-banner').forEach(el => {
      if (!_$('.lav-sidebar')) {
        el.classList.remove('lav-banner--expand')
        return;
      }

      if (!_$('.lav-sidebar--active')) {
        el.classList.add('lav-banner--expand')
        return;
      }

      if (_$('.lav-sidebar--active')) {
        if (el.offsetTop < _$('.lav-sidebar').offsetTop + _$('.lav-sidebar').offsetHeight + 500) {
          el.classList.remove('lav-banner--expand')
        } else {
          el.classList.add('lav-banner--expand')
        }
    ...
    el.innerHTML = /* html */ `
      <div class='lav-product lav-product--account'>
        ...
        <div class='lav-product__summary'>
          ...
          <div class='lav-product__buy'>View ${typeName} ${getSvg('arrow-right')}</div>
        </div>
  ...
  async function fetchProducts(gameName = 'fortnite', type = 'accounts') {
    ...
    // If no props, retry once after 1.5 seconds
    if (!props) {
    ...
    const config = {
      ...
      type: type,
      game: {
        poster: props.game.poster_url,
        descr: descr,
        url: data.url,
        currency_name: props.currencyUnit?.currency_name,
        unit: props.currencyUnit?.name
      },
      items: props[type]?.data || props.currencies?.data || []
    }

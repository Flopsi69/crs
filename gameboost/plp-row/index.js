(function () {
  ...
  const styles = /* css */ `
    ...
    .lav-pagination > .text-muted-foreground {
      display: none;
    }
  `
  ...
  function changeListing() {
    // if (_$('.lav-listing')) return;

    if (!_$('.lav-pagination')) {
      const paginationEl = _$('.relative .pt-36 .col-span-1 .min-w-0 > .px-2 > .flex-1.gap-y-4');
      paginationEl.classList.add('lav-pagination');
    }

    // const parentEl = _$('.relative .pt-36 .col-span-1 .min-w-0 .grid.gap-4.mt-4.xl\\:grid-cols-4');

    // if (!parentEl) return;

    // parentEl.classList.add('lav-listing');
    // parentEl.classList.remove('xl\:grid-cols-4');
    // parentEl.classList.add('xl\:grid-cols-5');

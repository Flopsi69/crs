...
const quiz = [
  ...
  },
  {
    ...
    variants: [
      ...
      {
        id: 7,
        title: 'I don’t have preferences and want to practice different styles',
      },
    ],
  },
  {
...
function initQuiz() {
  ...
  function initStep(step, isBack) {
    ...
    function resetStep() {
      ...
      $$('.progress__line-item').forEach((el) => el.classList.remove('active'));
  ...
  function fillList() {
    ...
    for (const step of quiz.slice(1)) {
      ...
      $('.drop__values', stepItem).addEventListener('click', function (e) {
        e.preventDefault();
        if ($('.drop.active') && this.closest('.drop') !== $('.drop.active')) {
          $('.drop.active').classList.remove('active');
        }

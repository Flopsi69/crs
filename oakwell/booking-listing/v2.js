...
let isDisableTrakingVisibility = true
  ...
  const packages = [
    {
      ...
      list: [
        'VIP experience for two',
        '90-minutes private spa suite access',
    ...
    },
    {
      ...
      briefList: ['90-minute private spa suite access'],
      descr:
      ...
      feedback: `We had an incredible time, simple as that. We were in heaven from beginning to end. Beers on tap were fantastic, the service was excellent. The rooms (as you can see from the photos) feel like your own private escape. Highly recommend the Zero Gravity massage at the end, it adds more to the experience than you’d think. Thanks for an incredible evening Oakwell Beer Spa, we will be back!`,
      feedbackCaption: 'Uri H.',
      details: {
        title: 'Package Details:',
        descr:
...
function changeLayout(packages) {
  ...
  _$$('.services__item').forEach((item) => {
    ...
    const packageEl = _$(`.lavp[data-id="${package.id}"]`)

    packageEl.addEventListener('click', (e) => {
      _$('.lav-pop__btn-wrap .lav-pop__btn').dataset.id = package.id

      if (!e.target.closest('.lavp__details')) {
        ...
        packageEl.closest('.lavp').classList.add('active')

        fillPopAddons()
        // setTimeout(() => {
        //   el.closest('.lavp').classList.remove('active')
        // }, 1200)
        _$('.lav-sticky .lavd-hero__price').innerText = package.price
        _$('.lav-sticky .lavd-hero__price-per').innerText = package.pricePer
        _$('.lav-sticky').style.backgroundColor = package.check
        Modal.open('.lav-package')
        setTimeout(() => {
  ...
  function getPackageMarkup(package) {
    const packageMarkup = /* html */ `
      ...
      >
        ...
        <div class='lavp__list' style="display: ${
          !package.briefList ? 'none' : 'flex'
        }">
          ${package.briefList
            ?.map(
              (item, index) =>
                `<div class='lavp__list-item'>
                  ${getSvg('whiteCheck', package.check)}${item}
                  ${
                    index === package.briefList.length - 1 &&
                    package.briefListMore
                      ? '<span>+' + package.briefListMore + ' more</span>'
                      : ''
                  }
                </div>`
            )
        ...
        <div class='lavp__footer'>
          ...
          <button class='lavp__details ff-dm-sans'>Select</button>
        </div>
      </div>
    `
...
function fillPopAddons() {
  ...
  const addons = addonsConfig[_$('.lav-pop__btn-wrap .lav-pop__btn').dataset.id]
...
function addModalMarkup(packages) {
  ...
  _$('.lav-modal').insertAdjacentHTML(
    'beforeend',
    /* html */ `
    <div class="lav-modal__close">${getSvg('close2')}</div>
  `
  )
  ...
  packages.forEach((package, index) => {
    ...
    visibilityEvent(
      `.lavd-pack[data-id="${package.id}"] .lavd-hero__footer`,
      () => {
        _$('.lav-sticky').classList.remove('active')
        _$('.lavd-pack.active').classList.remove('lavd-pack_sticky')
      },
      () => {
        if (isDisableTrakingVisibility) return
        _$('.lav-sticky').classList.add('active')
        _$('.lavd-pack.active').classList.add('lavd-pack_sticky')
      },
      {
        once: false,
        threshold: 0
    ...
    const packageMarkup = /* html */ `
      ...
      package.id
        ...
        <div class='lavd-hero'>
          ...
          <div class='lavd-slider swiper__wrap'>
            <div class='swiper'>
          ...
          <div class='lavd-hero__footer'>
            ...
            <button class='lavd-hero__select ff-dm-sans'>Select Package</button>
          </div>
        </div>

        <div class='lavd-list__wrap'>
          <div class="lavd-details__title">Package INCLUDES:</div>
        ...
        <div class='lavd-feedback'>
          <div class='lavd-feedback__inner'>
            <div class='lavd-feedback__icon'>
              <img src='${config.dir}/img/leaf-left.svg' />
            </div>
            <div class='lavd-feedback__info'>
              <div class='lavd-feedback__text'>${package.feedback}</div>
            </div>
            <div class='lavd-feedback__icon'>
              <img src='${config.dir}/img/leaf-right.svg' />
            </div>
          </div>
          <div class='lavd-feedback__caption'>${package.feedbackCaption}</div>
        </div>

        <div class='lavd-details'>
          <div class='lavd-details__section'>
            <div class='lavd-details__title'>Package Description:</div>
...
function selectModalPackage() {
  const id = _$('.lav-pop__btn-wrap .lav-pop__btn').dataset.id
...
class Modal {
    ...
    this.el.insertAdjacentHTML(
      ...
      `
        <div class='lav-sticky'>
         <div class='lavd-hero__footer'>
            <div class='lavd-hero__footer-price'>
              <div class='lavd-hero__price'></div>
              <div class='lavd-hero__price-per'></div>
            </div>
              
            <button class="lav-sticky__btn ff-dm-sans">Select Package</button>
          </div>
        </div>
      `
    )
  ...
  static close(cb) {
    isDisableTrakingVisibility = true
...
function visibilityEvent(el, cb, cbRemove, customConfig = {}) {
  const config = {
    threshold: 0.3,
    timer: null,
    once: true,
  ...
  const svgObj = {
    whiteCheck: `
      <svg xmlns="http://www.w3.org/2000/svg" width="12" height="13" viewBox="0 0 12 13" fill="none">
        <path d="M6 0.5C4.81331 0.5 3.65328 0.851894 2.66658 1.51118C1.67989 2.17047 0.910851 3.10754 0.456725 4.2039C0.00259972 5.30026 -0.11622 6.50665 0.115291 7.67054C0.346802 8.83443 0.918247 9.90352 1.75736 10.7426C2.59648 11.5818 3.66557 12.1532 4.82946 12.3847C5.99335 12.6162 7.19975 12.4974 8.2961 12.0433C9.39246 11.5891 10.3295 10.8201 10.9888 9.83342C11.6481 8.84673 12 7.68669 12 6.5C12 4.9087 11.3679 3.38258 10.2426 2.25736C9.11742 1.13214 7.5913 0.5 6 0.5ZM9.41 5.0525L5.645 9.3225C5.58424 9.39095 5.51056 9.44673 5.42819 9.48662C5.34582 9.52652 5.25638 9.54976 5.165 9.555H5.12C4.94947 9.55605 4.78463 9.49368 4.6575 9.38L2.6425 7.5925C2.50393 7.4695 2.41989 7.2965 2.40887 7.11154C2.39785 6.92658 2.46076 6.74482 2.58375 6.60625C2.70675 6.46767 2.87975 6.38363 3.06471 6.37261C3.24967 6.3616 3.43143 6.4245 3.57 6.5475L5.07 7.87L8.3725 4.12C8.49516 3.98109 8.66798 3.8966 8.85294 3.88512C9.0379 3.87363 9.21984 3.93609 9.35875 4.05875C9.49766 4.18141 9.58215 4.35423 9.59363 4.53919C9.60512 4.72415 9.54266 4.90609 9.42 5.045L9.41 5.0525Z" fill="${color}"/>
      </svg>
    `,
    close2: `
      <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none">
        <circle cx="20" cy="20" r="20" fill="white"/>
        <path d="M13.5574 13.7219L26.2853 26.4498M13.559 26.3156L26.2869 13.5877" stroke="#134D3A" stroke-width="2" stroke-linecap="round"/>
      </svg>
    `,
    close: `

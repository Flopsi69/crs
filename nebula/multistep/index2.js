...
function initControl() {
  const controlEl = `
    <div class="lav-control">
      ...
      <button class="lav-control__next btn btn-primary">Continue to payment</button>
    </div>
  `;
  ...
  document
    ...
    .addEventListener('click', function () {
      if (step === 1) {
        gaEvent('Button. Continue to payment', 'Step: Information');
      } else if (step === 2) {
...
function handlePaypalErrors() {
  ...
  if (
    ...
    if (
      ...
      document.querySelector('.lav-paypal__choose').classList.add('lav-hide');
    }
  } else {
    document.querySelector('.lav-paypal__choose').classList.remove('lav-hide');
  }
}
...
function handlePaylaterErrors() {
  if (
    ...
    if (
      ...
      Array.from(document.querySelectorAll('.plans__value')).some(
        (item) => item.innerText.trim() !== 'Lifetime'
      )
    ) {
      document.querySelector('.lav-later').classList.add('lav-hide');
      textEl.innerText = `Only available for Lifetime Subscription purchases.`;

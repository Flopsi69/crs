...
.lav-estimate__details--solo .lav-estimate__cost {
  display: flex;
  align-items: center;
  gap: 10px;
}
...
.lav-estimate__cost-item:not(.active):hover {
  opacity: .8;
}
...
function addSitcky() {
  ...
  _$('.lav-sticky__btn').addEventListener('click', () => {
    pushDataLayer(
      'exp_imp_cart__button_03',
      'Scroll to Checkout',
      'click',
      'Sticky'
    )
    _$('.lav-summary').scrollIntoView({ behavior: 'smooth' })
  })
...
async function calculateShipping() {
  ...
  _$(
    '.lav-estimate__details:not(.lav-estimate__details--solo) .lav-estimate__cost'
  ).innerHTML = ''
  _$('.lav-shipping__price').innerText = '...'
  ...
  try {
    ...
    shippingRates.forEach((rate, index) => {
      ...
      if (index === 0) {
        ...
        _$('.lav-estimate__details--solo .lav-estimate__cost span').innerText =
          currency + rate.price
      }
      ...
      _$(
        '.lav-estimate__details:not(.lav-estimate__details--solo) .lav-estimate__cost'
      ).appendChild(rateItem)
    })

    if (shippingRates.length > 1) {
      $('.lav-estimate__details:not(.lav-estimate__details--solo)').slideDown()
    } else {
      $('.lav-estimate__details--solo').slideDown()
    }
...
function addShippingEstimate() {
  ...
  const markup = /* html */ `
    <div class='lav-estimate'>
      ...
      <div class='lav-estimate__details lav-estimate__details--solo' style='display: none;'>
        <div class='lav-estimate__cost'>
          ${getSvg('truck2')} Your estimated shipping cost: <span></span>
        </div>

...
class Modal {
  ...
  static addStyles() {
    const styles = /* css */ `
      ...
      .lavm__slider .owl-item:hover:before {
...
function checkSessionProduct(url) {
  const products = JSON.parse(sessionStorage.getItem('lavProducts')) || []

  return products.includes(url)
}

function addSessionProduct(value) {
  const products = JSON.parse(sessionStorage.getItem('lavProducts')) || []

  products.push(value)

  sessionStorage.setItem('lavProducts', JSON.stringify(products))
}
...
function handleExitIntent() {
        ...
        waitFor(
          () =>
            ...
            JSON.parse(localStorage['mage-cache-storage'])?.cart.items[0]
              ?.product_image,
          () => {
            const href = node.querySelector('.name a')?.href
            if (href) {
              addSessionProduct(href)
            }
  ...
  function addProducts() {
    ...
    function addProduct({
      ...
      if (!checkSessionProduct(product_url)) return false
  ...
  function openModal() {
    ...
    if (JSON.parse(localStorage['mage-cache-storage'])?.cart?.items?.length) {
      ...
      if (!document.querySelector('.lavm__slide')) {
        console.log('Trigger fire but no products')
        return
      }

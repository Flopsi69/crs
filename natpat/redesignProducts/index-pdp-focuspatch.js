(function () {
  ...
  const styles = /* css */ `
    ...
    #purchase .product-form__input--pill {
      margin-top: 36px;
    }
    #pdpGetNow .days {
      margin: 0;
    ...
    .product-form__submit:not(.lav-active) {
      pointer-events: none;
    ...
    #purchase .product-form__input--pill, #purchase .stay-container {
    ...
    @media(max-width: 768px) {
      #pdpGetNow .free-shipping-checkout {
        margin-bottom: 12px;
      }
      .lav-plans {
        margin-top: 16px;
      ...
      #pdpGetNow .js-heading + .lp-fp--mobile {
        display: none;
      }
    }
  ...
  async function initExp() {
    await waitFor(() => document.head && document.body && _$('#pdpGetNow .product__title'), false, { ms: 20 })
  ...
  async function checkChangePrice() {
    await waitFor('.rtx-subscription-label__wrapper', false, { ms: 20 })
    ...
    observer.observe(_$('.rtx-subscription-label__wrapper'), { childList: true, subtree: true, attributes: true })
  }

  async function createNewPlans() {
    await waitFor(() => _$(`[data-pack="1"] .each-pack`).dataset.onetime && Shopify.money_format, false, { ms: 20 })
    // Hide prices by default
    const packagesEl = _$('#pdpGetNow variant-selects');

    const markup = /* html */ `<div class='lav-plans'></div>`
    packagesEl.insertAdjacentHTML('beforebegin', markup);

    _$$('label', packagesEl).forEach(el => {
    ...
    function getPlanMarkup(el) {
      const isBages = el.dataset.pack == "3";

      const patches = _$('.number-patches', el).innerText.trim() + ' patches';
      const title = el.previousElementSibling.value;
      ...
      planEl.addEventListener('click', function (e) {
        if (e.target.closest('.rtx-subscription-dropdown')) {
        ...
        if (!_$('.product-form__submit.lav-active')) {
          _$('.price.price--large').removeAttribute('style');
          _$('.product-form__submit').classList.add('lav-active');
          _$('.product-form__submit').innerText = 'Proceed to Checkout';
          ...
          _$('.product-form__submit.lav-active').addEventListener('click', () => {
            pushDataLayer('exp_fp_proceed_to_checkout', 'Proceed to checkout', 'click', 'Purchase section');
          });
        }

        if (countPacks !== 1) {
          const selectDeliveryEl = _$('[data-retextion-subscription-box]');
  ...
  function updateUi() {
    _$('.product-form__submit').innerText = 'Choose Quantity To Proceed';
    setTimeout(() => {
      _$('.product-form__submit').innerText = 'Choose Quantity To Proceed';
    }, 500);
    setTimeout(() => {
    ...
    waitFor(() => _$('.price.price--large'), () => {
      _$('.price.price--large').style.setProperty('display', 'none', 'important');
    })
  }

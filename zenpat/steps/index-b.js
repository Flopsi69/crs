...
const config = {
  ...
  clarity: ['set', 'exp_zen_slider_b', 'variant_1'],
  debug: false
}
...
.lav-acc__head:before {
  ...
  width: 24px;
  height: 24px;
  background: url('${config.dir}/img/dropdown-arrow.svg') center no-repeat;
  background-size: contain;
  transition: 0.35s;
}
.lav-acc__item.active .lav-acc__head:before {
...
function addAccordion() {
  ...
  function fillList(item, index) {
    ...
    if (index === 1) {
      ...
      el.querySelector('.lav-acc__body').style.display = 'block'
    }
    ...
    el.querySelector('.lav-acc__head').addEventListener('click', () => {
      const $el = $(el.closest('.lav-acc__item'))

      $('.lav-acc__item.active')
        .not($el)
        .removeClass('active')
        .find('.lav-acc__body')
        .slideUp()

      $el.toggleClass('active').find('.lav-acc__body').slideToggle()

      if ($el.hasClass('active')) {
        pushDataLayer(
          'exp_zenpatch_slider_click_02',
          'Expand',
          'Button',
          `How it works - Steps ${index}: ${item.title}`
        )
      ...
      if (isIntersecting) {
        config.timer = setTimeout(() => {
          if (
            ...
            (!target.classList.contains('lav-acc__item') ||

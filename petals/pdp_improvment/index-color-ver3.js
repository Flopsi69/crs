;(function () {
  ...
  if (location.href.includes('/pages/faq')) {
    hideParapraphExp();
    return;

    async function hideParapraphExp() {
      await waitFor(() => '.product__description', false)
    
      let findText = 'Can I change the color of my stone?';

      const findParagraph = _$$('.scroll-trigger p', document, true)?.find(p => {
        return p.innerText.includes(findText)
      })

      if (!findParagraph) return

      findParagraph.nextElementSibling?.remove();
      findParagraph.remove();
    }
  }
...
.lav-selector__toggler .ringSize_guide p {
  margin-bottom: 0;
...
.lav-selector:not([data-type="size"]) .lav-selector__toggler .form__label:after {
  content: ':';
}
...
@media(max-width: 767px) {
  .lav-measure-container .select:before, .lav-measure-container .select:after {
    display: none!important;
  }
  #satcb_bar {
    display: none!important;
...
.lav-selector__toggler .lav-color__image svg, .lav-sticky .lav-color__image svg {
  width: 12px;
  height: 12px;
  right: -1px;
  top: -1px;
...
@media(min-width: 768px) {
  ...
  .lav-measure-container .select .svg-wrapper svg {
    display: block;
    color: #000;
    right: 0;
    top: 0;
...
`
  ...
  async function hideExpText() {
    ...
    const findParagraph = _$$('.product__description p', document, true)?.find(p => {
      return p.innerText.includes('-Please leave us a note on the cart page if you would like to change the color of your stone')
    })

    if (!findParagraph) return

    findParagraph.innerHTML = findParagraph.innerHTML.replace('-Please leave us a note on the cart page if you would like to change the color of your stone<br>', '')
  }
  ...
  async function addColorsOptions() {
    ...
    const markup = /*html*/ `
      <div class="lav-colors ${is14K ? '' : 'lav-colors--disabled'}">
    ...
    async function addToSticky(value, label, color) {
      ...
      _$('.lav-selector-color').addEventListener('click', function (e) {
        ...
        function rerender() {
          ...
          _$('.lav-selector__toggler').innerHTML = headerHTML + bodyHTML + noteHTML
  ...
  function addNoteRecord() {
    ...
    const material = _$('.lav-material .jquery-grid-picker-item-selected a')?.innerText.trim()
    const size = _$('.lav-size .jquery-grid-picker-item-selected a')?.innerText.trim()
    let productInfo = ''
    ...
    if (size) {
      productInfo = productInfo ? productInfo + `,${size}` : `${size}`;
    }
    ...
    const newRecord = 'Stone color: ' + colorText + ' - ' + productTitle + productInfo ;
  ...
  function updateSizeAndLength() {
    ...
    if (sizeEl || lengthEl) {
      (sizeEl || lengthEl)?.closest('.product-form__input--dropdown').classList.add('lav-measure-container')
    }
        ...
        function rerender() {
          ...
          if (_$('.lav-selector__toggler #guide_desc')) {
            _$('.lav-selector__toggler #guide_desc').addEventListener('click', () => {
              ...
              _$('.section-header')?.classList.add('lav-selector-guide-opened')
              ...
              const interval = setInterval(() => {
                if (_$('#custom_notifications[style*="translateX(100%)"]')) {
                  if (_$('.lav-selector.active')) {
                    ...
                    _$('.section-header')?.classList.remove('lav-selector-guide-opened')
                  }
  ...
  function openSelector(type) {
    ...
    _$('.lav-selector').dataset.type = type;

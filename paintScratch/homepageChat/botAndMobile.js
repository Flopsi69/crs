...
const loadScriptOrStyle = (url) => {
  return new Promise((resolve, reject) => {
    ...
    if (type === 'js') {
      ...
      const loadedStyles = Array.from(document.styleSheets).map((style) =>
        style.href?.toLowerCase()
      );
...
class mobileDesign {
  ...
  paintCodePage() {
    ...
    $(
      '.action-button-orange, p+.panel-primary .btn-primary, #color_search_widget_copy button'
    ).append(btnArrowSvg);
    ...
    $('#select_year_copy').on('change', function () {
      pushDataLayer(
        'exp_mob_redes_dropdown_year_sel',
        'Year',
        'Dropdown',
    ...
    $('#select_make_copy').on('change', function () {
      pushDataLayer(
        'exp_mob_redes_dropdown_make_sel',
        'Make',
        'Dropdown',
        'Order Touch Up Paint'
      );
    ...
    $('#select_model_copy').on('change', function () {
      pushDataLayer(
        'exp_mob_redes_dropdown_model_sel',
        'Model',
        'Dropdown',
        'Order Touch Up Paint'
      );
    ...
    $('#color_search_widget_copy button').on('click', function () {
      pushDataLayer(
        'exp_mob_redes_btn_find_your_color',
        'Find Your Color',
        'Button',
        'Order Touch Up Paint'
      );
  ...
  productsPage() {
    ...
    if (this.page === '/cgi-bin/order-form.cgi') {
      ...
      $('.to_cart button').on('click', () => {
        ...
        pushDataLayer(
          'exp_mob_redes_view_cart_st_btn',
          'View Shopping cart',
          'Button',
          'Sticky section'
        );
      ...
      $('.slide_cart ul').on('click', '.minus', function () {
        ...
        pushDataLayer(
          'exp_mob_redes_slide_dec_qnt_btn',
          'Decrease qnt',
          'Button',
          'Slide cart'
        );
      ...
      $('.slide_cart ul').on('click', '.plus', function () {
        ...
        pushDataLayer(
          'exp_mob_redes_slide_inc_qnt_btn',
          'Inscrease qnt',
          'Button',
          'Slide cart'
        );
      ...
      $('#guided-results .orderform, #guided-results .additems').each(
        (i, item) => {
          ...
          $(item)
            .find('.descr .input_block span:first-of-type')
            .after($(item).find('input'));
    ...
    function createSliders() {
      let wrap = 0;
      $('#single-products-container .products-container > div').each(function (
        i,
        item
      ) {
        if ($(item).hasClass('category-heading')) {
          wrap++;
          $(item).after(
            `<div class="slider_wrapper_product" data-wrap="${wrap}"></div>`
          );
        } else {
    ...
    function addCartInfo(name) {
      ...
      $('body').append(/* html */ `
          <div class="add_to_cart_info">
            <p><span>${
              name.slice(0, 20) + ellipsis
            }</span> was added to Cart</p>
  ...
  cartPage() {
      ...
      $(item)
        ...
        .each(function (y, itemList) {
          ...
          <li>
            ...
            <div class="descr">
              <a href="${$(itemList).find('.item-title a').attr('href')}">${$(
            itemList
          )
            .find('.item-title a')
            .text()}</a>
        ...
        <div class="cart_block">
          <div class="color_head">
            ...
            <p>
              ...
              <span class="car_color">${$(item)
                .find('tr:nth-of-type(2) td[colspan]')
                .text()}</span>
            </p>
          </div>
    ...
    $('.change_btn').on('click', function () {
      pushDataLayer(
        'exp_mob_redes_btn_change_items',
        'Change items',
        'Button',
        'Cart'
      );
    ...
    $('.remove_btn').on('click', function () {
      pushDataLayer(
        'exp_mob_redes_btn_remove_vehicle',
        'Remove vehicle',
        'Button',
        'Cart'
      );
    ...
    $('.total_block .paypal').on('click', function (e) {
      e.preventDefault();
      pushDataLayer(
        'exp_mob_redes_btn_paypal_checkout',
    ...
    $('.total_block .checkout').on('click', function (e) {
      e.preventDefault();
      pushDataLayer(
        'exp_mob_redes_btn_secur_checkout',
        'Secure checkout',
        'Button',
        'Cart'
      );
    ...
    $('.cart_block .descr a').on('click', function () {
      pushDataLayer(
        'exp_mob_redes_btn_open_item_cart',
        'Open item',
        'Link',
        'Cart'
      );
  ...
  checkoutPage() {
    ...
    $(
      '#main>.billing-information:first-of-type input, #main>.billing-information:first-of-type select'
    ).on('change', function () {
      Address_Fill();
    });
  }
  ...
  headerChange() {
    ...
    $('.crs_steps_line .line p').css(
      'width',
      listWidth * step - listWidth / 2 + 'px'
    );
    ...
    setTimeout(() => {
      if (document.querySelector('#footer')) {
        document
          ...
          .insertAdjacentHTML('afterbegin', block);
      } else {
        document
          .querySelector('#bottom_footer_text')
          .insertAdjacentHTML('beforebegin', block);
      }

      $('.touch_links ul').on('click', 'a', function (e) {
        pushDataLayer(
          'exp_mob_redes_link_touch_up',
          $(this).text().trim(),
          'Link',
          'Touch Up'
        );
...
function initHomepageBot() {
  console.log('initHomepageBot');
...
`;
  ...
  async function initExp(stylesEl) {
  ...
  function createCarSelector() {
    ...
    el.insertAdjacentElement(
      'beforeend',
      _$('#select_year_div').cloneNode(true)
    );
    el.insertAdjacentElement(
      'beforeend',
      _$('#select_make_div').cloneNode(true)
    );
    el.insertAdjacentElement(
      'beforeend',
